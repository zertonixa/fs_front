...
describe("calendar utils", () => {
  it("buildMonthDays строит календарную сетку и отмечает дни текущего месяца", () => {
    ...
    const aprilDay = days.find((day) => day.date.toISOString().startsWith("2026-04-15"));
    const marchDay = days.find((day) => day.date.toISOString().startsWith("2026-03-29"));
    const mayDay = days.find((day) => day.date.toISOString().startsWith("2026-05-02"));
  ...
  it("для WASHING разрешает диапазон максимум в один день", () => {
    function Probe() {
      ...
      return (
        <div>
          ...
          <button type="button" onClick={() => range.handleDayClick(new Date("2026-04-24T00:00:00.000Z"))}>first</button>
          <button type="button" onClick={() => range.handleDayClick(new Date("2026-04-26T00:00:00.000Z"))}>too far</button>
        </div>
      );
    }
    ...
    expect(document.querySelector('[data-testid="max"]')?.textContent).toBe("1");
  ...
  it("для DRYING разрешает диапазон до трех дней и умеет проверять selected", () => {
    ...
    function Probe() {
      ...
      return (
        <div>
          <span data-testid="max">{range.maxRange}</span>
          <span data-testid="selected-start">{String(range.isSelected(new Date("2026-04-24T00:00:00.000Z")))}</span>
          <button type="button" onClick={() => range.handleDayClick(new Date("2026-04-24T00:00:00.000Z"))}>first</button>
          <button type="button" onClick={() => range.handleDayClick(new Date("2026-04-26T00:00:00.000Z"))}>second</button>
        </div>
      );
    }
    ...
    expect(document.querySelector('[data-testid="max"]')?.textContent).toBe("3");
    click(document.querySelectorAll("button")[0]);
    click(document.querySelectorAll("button")[1]);
...
describe("useSelectCso", () => {
  ...
  it("возвращает текущий ЦСО и список доступных без текущего", () => {
    function Probe() {
      ...
      return (
        <div>
          ...
          <button type="button" onClick={() => changeCso(1)}>change</button>
        </div>
      );
    }
    ...
    expect(document.querySelector('[data-testid="cso"]')?.textContent).toBe("4");
    expect(document.querySelector('[data-testid="available"]')?.textContent).toBe("1,3");
